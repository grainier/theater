...
public function addNewEvent (mod:Event event, string tracingParentSpanId) (json jsonResponse, error err) {
    endpoint<sql:ClientConnector> ep {
    }
    observe:Span span = observe:startSpan("Event Handling", "Confirming Add Event", {}, observe:ReferenceType.CHILDOF, tracingParentSpanId);
    bind sqlCon with ep;
    ...
    if (!existingID.equalsIgnoreCase("0")) {
        span.logError("DuplicateEvent", "Event with name " + event.name + " already exists");
    ...
    if (ret == 1) {
        ...
        span.log("EventAdded", "Event with name " + event.name + " created successfully.");
    } else {
